...
async function uploadImage(file: File): Promise<string> {
  ...
  // Le backend retourne { url: "/uploads/xxx.jpg" }
        ...
        <div className="max-w-7xl mx-auto px-8 py-8">
          <div className="flex items-start justify-between mb-8">
            <div>
              ...
              <p className={`text-sm ${dm ? 'text-gray-400' : 'text-gray-500'}`}>Voici ce qui se passe avec vos services aujourd'hui</p>
            </div>
            <button
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: dm ? '#1f2937' : '#fff', color: dm ? '#fff' : '#000', borderRadius: 20, padding: '32px 36px', width: 580, maxHeight: '92vh', overflowY: 'auto', boxShadow: dm ? 'none' : '0 24px 60px rgba(0,0,0,0.18)' }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  {(['select', 'new'] as const).map(mode => (
                    <button key={mode} type="button"
                      ...
                      style={{
                        ...
                        background: categoryMode === mode ? '#B91C1C' : dm ? '#374151' : '#F3F4F6',
                        color: categoryMode === mode ? '#fff' : dm ? '#D1D5DB' : '#6B7280',
                      }}
                    >
              ...
              <div>
                <p style={{ fontSize: 13, fontWeight: 600, color: dm ? '#F3F4F6' : '#374151', marginBottom: 8 }}>
                  Image du service
                </p>
                ...
                <div
                  ...
                  style={{
                    border: '2px dashed',
                    borderColor: imagePreview ? '#B91C1C' : dm ? '#374151' : '#E5E7EB',
                    borderRadius: 12,
                    padding: imagePreview ? 0 : '24px 16px',
                    textAlign: 'center',
                    cursor: 'pointer',
                    background: imagePreview ? 'transparent' : dm ? '#1f2937' : '#FAFAFA',
                    overflow: 'hidden',
                ...
                >
                  {imagePreview ? (
                    <div style={{ position: 'relative' }}>
                      ...
                      <div style={{ padding: '8px 12px', fontSize: 12, color: dm ? '#9CA3AF' : '#6B7280', background: dm ? '#111827' : '#F9FAFB' }}>
                        {imageFile?.name} — Cliquez pour changer
                      </div>
                    </div>
                  ) : (
                    <>
                      <div style={{ fontSize: 32, marginBottom: 8 }}>📁</div>
                      <p style={{ fontSize: 13, fontWeight: 600, color: dm ? '#F3F4F6' : '#374151', margin: '0 0 4px' }}>
                        Cliquez pour sélectionner une image
                      </p>
                      <p style={{ fontSize: 11, color: dm ? '#9CA3AF' : '#9CA3AF', margin: 0 }}>
              ...
              {/* Erreur */}
              {formError && (
                <p style={{ color: '#B91C1C', fontSize: 13, margin: 0, padding: '8px 12px', background: dm ? '#450A0A' : '#FEF2F2', borderRadius: 8 }}>
                  {formError}
                </p>
              )}
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
                <button
                  ...
                  style={{
                    ...
                    opacity: submitting || imageUploading ? 0.7 : 1,
                  }}
                >
                ...
                <button
                  ...
                  style={{
                    padding: '12px 20px', background: dm ? '#374151' : '#F3F4F6', color: dm ? '#F3F4F6' : '#374151',
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className={`border-b ${dm ? 'border-gray-800' : 'border-gray-100'}`}>
                          <th className={`text-left px-6 py-4 text-xs font-bold uppercase tracking-wider ${dm ? 'text-gray-400' : 'text-gray-500'}`}>Service</th>
                          <th className={`text-left px-6 py-4 text-xs font-bold uppercase tracking-wider ${dm ? 'text-gray-400' : 'text-gray-500'}`}>Prix</th>
                          <th className={`text-left px-6 py-4 text-xs font-bold uppercase tracking-wider ${dm ? 'text-gray-400' : 'text-gray-500'}`}>Statut</th>
                          <th className={`text-left px-6 py-4 text-xs font-bold uppercase tracking-wider ${dm ? 'text-gray-400' : 'text-gray-500'}`}>Commandes</th>
                        </tr>
                      </thead>
                      <tbody>
                              ...
                              <td className="px-6 py-4">
                                <div className="flex items-center gap-3">
                                  <div style={{
                                    ...
                                    background: imageUrl ? `url(${imageUrl}) center/cover` : dm ? '#1f2937' : '#FEF2F2',
            ...
            <div>
              <div className={`border rounded-2xl overflow-hidden ${dm ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'}`}>
                <div className={`px-6 py-5 border-b ${dm ? 'border-gray-800' : 'border-gray-100'}`}>
                  <h3 className={`text-lg font-bold ${dm ? 'text-white' : 'text-gray-900'}`}>Commandes récentes</h3>
                </div>
                <div className={`divide-y ${dm ? 'divide-gray-800' : 'divide-gray-100'}`}>
                  {orders.length === 0 ? (
                    <div className="px-6 py-8 text-center text-sm text-gray-500">Aucune commande pour l'instant.</div>
                  ) : orders.map(order => (
                    <div key={order.id} className={`px-6 py-4 transition-colors cursor-pointer ${dm ? 'hover:bg-gray-800/50' : 'hover:bg-gray-50'}`}>
                      <div className="flex items-start gap-3 mb-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className={`text-xs font-bold truncate ${dm ? 'text-white' : 'text-gray-900'}`}>{order.serviceTitle}</p>
                          <p className={`text-[10px] mt-0.5 ${dm ? 'text-gray-400' : 'text-gray-500'}`}>{order.clientName}</p>
                        </div>
                      </div>
                      <div className="flex items-center justify-between mb-2">
                        <span className={`text-xs font-bold ${dm ? 'text-white' : 'text-gray-900'}`}>{order.servicePrice} TND</span>
